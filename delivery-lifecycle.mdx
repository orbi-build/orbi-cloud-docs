...
`ai-blocked` is a deliberate stop, not a crash. Orbi has evaluated the situation, determined that automatic recovery is not safe, and is waiting for a human to decide what happens next. The comment Orbi leaves on the Issue explains exactly why it stopped.
