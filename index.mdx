...
flowchart TD
  subgraph setup["Setup — once"]
    direction LR
    s1["1 Sign in<br/>with GitHub"] --> s2["2 Install the<br/>GitHub App"] --> s3["3 Connect one<br/>repository"] --> s4["4 Provisioning<br/>~1–2 min"]
  end
  ...
  subgraph rel["Release — only when you say so"]
    direction LR
    r1["6 You type a<br/>version"] -.-> r2["Gates, CI wait,<br/>bump, tag"] --> r3["GitHub Release<br/>milestone closed"]
  end

  setup --> work --> rel
```

The two dotted arrows are the only two switches you throw: the `ai-ready` label, and the version number. Orbi never throws either one for you.
